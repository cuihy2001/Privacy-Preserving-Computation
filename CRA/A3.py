...
CLIENT_IP = "192.168.3.140"   # 当前设备的IP
PROVER3_IP= "192.168.3.126"
PROVER4_IP= "192.168.3.144"
TOPIC_ALL = "rpi/all"        # 广播频道
TOPIC_SERVER = "rpi/server/3"   # 接收客户端消息的频道
TOPIC_CLIENT = f"rpi/client/{CLIENT_IP}"  # 当前客户端的专属频道
TOPIC_PROVER3 = f"rpi/client/{PROVER3_IP}"  # 当前证明者的专属频道
TOPIC_PROVER4 = f"rpi/client/{PROVER4_IP}"  # 当前证明者的专属频道
    ...
    if msg.topic == TOPIC_SERVER:
        ...
        if msg_type == "Ch":
            ...
            if data['T']['T_exp'] < t:
                ...
                # 发送data
                client.publish(TOPIC_PROVER3, json.dumps(data))
                client.publish(TOPIC_PROVER4, json.dumps(data))
